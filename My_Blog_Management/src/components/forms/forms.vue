<template>
    <div class="form">
        <yk-space dir="vertical" size="xl">
            ...
            <input type="text" class="form-title" placeholder="请输入标题" v-model="formData.Title" style="width: 520px"/>
            <yk-space align="center">
                <div class="subset">
                    ...
                    <yk-dropdown @selected="subsetSelect">
                        <template v-for="item in subsetList" :key="item.id">
                            <yk-dropdown-item :value="item.id">
                                {{ item.subsetName }}
                            </yk-dropdown-item>
                        </template>
                    </yk-dropdown>
                ...
                <yk-space align="center" size="s">
                    <yk-tag v-for="(tag, index) in formData.Label" :key="index" closeable shape="round"
        ...
        <div class="form-cover" v-if="props.Classify == 0">
            <yk-upload :uploadUrl="uploadUrl" :file-list="fileUrl" :limit="1" accept="images/*" desc="封面800x600"
                @handleSuccess="handleSuccess" @handleDelete="deletePhoto"></yk-upload>
        </div>
...
const selectLabel = (e: string) => {
    ...
    if (formData.value.Label!.length < 3) {
        if(!formData.value.Label.includes(e))
